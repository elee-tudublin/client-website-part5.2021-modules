...
let getProducts = async () => {
  try {
    ...
    return await api.getDataAsync(`${api.BASE_URL}/product`);
...
let getCategories = async () => {
  try {
    ...
    return await api.getDataAsync(`${api.BASE_URL}/category`);
...
let getProductsByCategory = async (id) => {
  ...
  try {
    ...
    products = await api.getDataAsync(`${api.BASE_URL}/product/bycat/${id}`);
  } // catch and log any errors
  catch (err) {
    console.log(err);
  }
...
let deleteProductById = async (id) => {
  ...
  const url = `${api.BASE_URL}/product/${id}`;
